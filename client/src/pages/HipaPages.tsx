import { ArrowRight, Check, ChevronDown, ChevronLeft, ChevronRight, Heart, Leaf, Mail, MapPin, Package, Phone, ShieldCheck, Sparkles, Truck } from "lucide-react";
...
export function HomePage() {
  ...
  useEffect(() => {
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    const onScroll = () => setHeroParallax(Math.min(window.scrollY * 0.12, 48));
    onScroll();
...
export function B2BEnquiriesPage() { return <><Breadcrumbs current="Product Enquiry" /><section className="contact-hero"><div className="container contact-hero-inner"><p className="eyebrow">Business Product Enquiries</p><h1>HIPA Masalas Product Information</h1><p>Use this form to request current product information and make an enquiry relevant to your business.</p></div></section><section className="b2b-enquiry-section"><div className="container b2b-enquiry-grid"><div className="b2b-enquiry-copy"><p className="eyebrow">Before You Submit</p><h2>Help HIPA Masalas understand your enquiry</h2><p>Share the product you are interested in, your business type and your city or region. You can also include an expected monthly volume if it is known.</p><ul><li>Choose the current product you want to ask about.</li><li>Select the business type that best describes your enquiry.</li><li>Add your city or region for context.</li><li>Use the message field for any further product questions.</li></ul><p className="b2b-enquiry-note">HIPA Masalas will respond with current product information relevant to the enquiry. Commercial availability, pricing, delivery and service details are shared only when confirmed.</p></div><div className="contact-form-card b2b-enquiry-form"><h2>Send an Enquiry</h2><p>Fields marked * are required.</p><EnquiryForm formId="b2b-enquiry" variant="distributor" /></div></div></section></>; }

export function PrivacyPage() { return <><Breadcrumbs current="Privacy" /><section className="journal-replica"><div className="container" style={{ maxWidth: 840 }}><p className="eyebrow">Privacy</p><h1>Privacy Policy</h1><p className="section-desc">HIPA Masalas uses the details submitted through this website to respond to enquiries and record newsletter subscription requests.</p><div className="article-body"><h2>Information submitted through this website</h2><p className="section-desc">Enquiry forms may collect a name, mobile number, email address, business type, product interest, message and, where provided, city or region and expected monthly volume. Newsletter subscriptions collect an email address and consent.</p><h2>How HIPA Masalas uses this information</h2><p className="section-desc">HIPA Masalas uses submitted details to respond to the relevant request, manage product enquiries and record consent for newsletter updates. You can contact HIPA Masalas through the Contact page to ask about information you have submitted.</p><h2>Consent</h2><p className="section-desc">The enquiry and newsletter forms require consent before submission. Do not submit information you do not wish HIPA Masalas to use for the stated purpose.</p></div></div></section></>; }
