...
export function AdminPasswordLogin({ title, description }: { title: string; description: string }) {
  ...
  return <main className="admin-blog-shell"><section className="admin-auth-card"><p className="admin-kicker">HIPA Masala</p><h1>{title}</h1><p>{description}</p><form className="admin-blog-form" autoComplete="off" onSubmit={submit}><label>Username<input autoComplete="off" value={username} required maxLength={128} onChange={(event) => setUsername(event.target.value)} /></label><label>Password<input autoComplete="new-password" type="password" value={password} required maxLength={256} onChange={(event) => setPassword(event.target.value)} /></label>{login.isError ? <p className="admin-message error" role="alert">Username or password is incorrect.</p> : null}<button className="admin-primary-button" type="submit" disabled={login.isPending}><LogIn size={17} />{login.isPending ? "Signing in…" : "Sign in"}</button></form></section></main>;
}
